...
class DBHelper(context : Context) : SQLiteOpenHelper(context, "database.db", null, 1){
    ...
    // Passar os camando de criação da tabela
    ...
    // Criando o cache das informações
    fun CriarCache( heroi : Herois ){


    }

    // Função de inserção de heróis
    ...
    fun ListarHerois() : List<Herois> {
        ...
        if( lerRegistros.count > 0 ){
           ...
           lerRegistros.moveToFirst()
            do{
                // Procurando a referencia da coluna
                ...
                // Extrair o valor da minha coluna
